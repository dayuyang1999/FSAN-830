...
corruptDF1 = corruptDF %>%
  group_by(region)%>%
  mutate(rank_region = dense_rank(desc(population)))%>%
  filter(rank_region <= 10)

tmp = corruptDF %>%
  group_by(region)%>%
  mutate(rank_region = rank(population, decreasing=TRUE))%>%
  filter(rank_region <= 10, region == "Americas")
  
corruptDF2 = corruptDF1 %>%
...
plot7 = plot5 +
  geom_text_repel(aes(label = cLabel),
                  ...
                  point.padding = 0.75,
                  fontface = "bold")+
  geom_text(label = "Uruguay", x=44, y=0.81, color="darkred")
plot7
# corruptDF[corruptDF$country=="Belarus",]
...
colorList2 = c("Asia Pacific" = "#DE2910", # based on China Red
             ...
             "SSA" = "#FF4500", # based on Nigeria Green
...
ggsave("YangEconPlot.pdf", width = 11, height = 8.5) 
